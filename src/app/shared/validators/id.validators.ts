...
export function amkaValidator(): AsyncValidatorFn {
...
export function afmValidator(): AsyncValidatorFn {
  return (control: AbstractControl): Observable<ValidationErrors | null> => {
    const value = String(control.value ?? '').replace(/\s/g, '');
    if (value === '') {
      return of(null);
...
export function licenceNumberValidator(): AsyncValidatorFn {
  return (control: AbstractControl): Observable<ValidationErrors | null> => {
    ...
    return timer(300).pipe(
      switchMap(() =>
        of(/^[A-Za-zΑ-Ωα-ω0-9-]{5,20}$/.test(value) ? null : { licenceFormat: true })
      )
    );
  };
}
